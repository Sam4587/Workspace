...
# 创建批处理版本快捷方式（兼容性更好）
$Shortcut2 = $WshShell.CreateShortcut("$DesktopPath\Publisher Tools Start(BAT).lnk")
$Shortcut2.TargetPath = "$ScriptPath\start-services.bat"
$Shortcut2.WorkingDirectory = $ScriptPath
$Shortcut2.WindowStyle = 1
$Shortcut2.IconLocation = "shell32.dll,14"
$Shortcut2.Description = "Start Publisher Tools Services (Batch Version)"
$Shortcut2.Save()

# 创建停止服务快捷方式
$Shortcut3 = $WshShell.CreateShortcut("$DesktopPath\Publisher Tools Stop.lnk")
$Shortcut3.TargetPath = "$ScriptPath\stop-services.bat"
$Shortcut3.WorkingDirectory = $ScriptPath
$Shortcut3.WindowStyle = 1
$Shortcut3.IconLocation = "shell32.dll,50"  # 使用红色停止图标
$Shortcut3.Description = "Stop Publisher Tools Services"
$Shortcut3.Save()
...
Write-Host "2. Publisher Tools Start(BAT).lnk - Batch version starter" -ForegroundColor White
Write-Host "3. Publisher Tools Stop.lnk - Service stopper" -ForegroundColor White
Write-Host ""
Write-Host "Double-click the shortcuts to quickly start/stop services" -ForegroundColor Cyan
Write-Host ""

# 询问是否立即测试
$choice = Read-Host "Test start services now? (Y/N)"
if ($choice -eq 'Y' -or $choice -eq 'y') {
    Write-Host "Starting services..." -ForegroundColor Yellow
    Start-Process -FilePath "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -File `"$ScriptPath\start-services.ps1`"" -WindowStyle Normal
}
