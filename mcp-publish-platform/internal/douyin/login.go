...
const (
	douyinCreatorHome  = "https://creator.douyin.com/creator-micro/home"
	douyinPublishURL   = "https://creator.douyin.com/creator-micro/publish/publish"
	douyinVideoPublish = "https://creator.douyin.com/creator-micro/publish/upload"
)
...
func (a *LoginAction) CheckLoginStatus(ctx context.Context) (bool, error) {
	...
	if err := pp.Navigate(douyinCreatorHome); err != nil {
		return false, errors.Wrap(err, "导航到抖音创作者中心失败")
	}
	...
	hasLoginBtn, _, err := pp.Has(`.login-button, .login-btn, [class*="login"]`)
	if err != nil {
		logrus.Debugf("检查登录按钮失败: %v", err)
	}
	...
	hasUserInfo, _, err := pp.Has(`.user-info, .user-name, .avatar, [class*="user"]`)
	if err != nil {
		logrus.Debugf("检查用户信息失败: %v", err)
	}
	...
	hasQRCode, _, _ := pp.Has(`.qrcode, [class*="qrcode"], [class*="qr-code"]`)
	if hasQRCode {
...
func (a *LoginAction) Login(ctx context.Context) error {
	...
	logrus.Info("正在导航到抖音创作者中心...")
	if err := pp.Navigate(douyinCreatorHome); err != nil {
		return errors.Wrap(err, "导航到抖音创作者中心失败")
	}
...
func (a *LoginAction) checkLoginSuccess(page *rod.Page) (bool, error) {
	...
	if currentURL != "" && 
	   ...
	   containsString(currentURL, "creator.douyin.com") {
